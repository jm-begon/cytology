...
//    restart= true
//
//    //union full view (multithreaded) to union "inter-area" annotations
//
//        max = 100
//        def maxWithArea = maxWith
//        def maxHeightArea = maxHeight
//
//        while(restart && (max>0)) {
//
//                if(maxWithArea>1 && maxHeightArea>1) {
//                    maxWithArea--
//                    maxHeight--
//                    //decale les carrés a merger en réduisant le nombre d'area
//                    areas = computeArea(cytomine,image,maxWithArea-1,maxHeight-1)
//            //        def result = unionArea(cytomine,image,user,term,bufferLength,minIntersectLength,areas,false)
//                    def result = unionArea(cytomine,image,user,term,bufferLength,minIntersectLength,areas,true)
//                    restart = result.restart
//                    unionedAnnotation.addAll(result.unionedAnnotation)
//                    max--
...
// private List intersectAnnotationFull(def cytomine, def image, def user, def term, def bufferLength, def minIntersectLength,def threads) {
//     def data = []
//     println "intersectAnnotation..."
//
//     def filters = [:]
...
//     println "annotationsMap=${annotationsMap.size()}"
//
//     def listArray = splitList(annotationsMap.entrySet().asList(),threads)
//     listArray.eachWithIndex { l,i->
//         println "List $i = ${l.size()} items"
//     }
//
//     def listThread = []
//
...
//         def th = Thread.start {
//
//             l.eachWithIndex { current,indice ->
//                 if(indice%50==0) {
//                     println "$i => $indice/${l.size()}"
//                 }
