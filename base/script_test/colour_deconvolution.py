...
def deconvolve_im_array(im_array, MOD_vectors):
    ...
    if (alpha_mask != None):
        ...
        np.dstack( (stains[0], alpha_mask.astype(np.uint8)))
        np.dstack( (stains[1], alpha_mask.astype(np.uint8)) )
        np.dstack( (stains[2], alpha_mask.astype(np.uint8)) )
    
        return stains
    else:
        return stains

def deconvolve_dir(src_dir, dest_dir, MOD_vectors, stain):
    ...
    for c in os.listdir(src_dir):
        ...
        for _file in os.listdir(os.path.join(src_dir, c)):
            ...
            print im_src.shape
            im_dest = deconvolve_im_array(im_src, MOD_vectors)[stain]
